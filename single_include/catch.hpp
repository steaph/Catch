/*
 *  CATCH-VC6 v0.9 build 33 (integration branch)
 *  Generated: 2013-04-13 22:27:46.061000
 *  ----------------------------------------------------------
    ...
    void LineWrapper::wrapInternal( std::string const& _str ) {
        ...
        for( std::size_t pos = 0; pos < _str.size(); ++pos ) {
            if( _str[pos] == '\n' )
            {
                ...
                nextTab = tab = 0;
                wrapInternal( _str.substr( pos+1 ) ); return ;
            }
            if( pos == width-tab ) {
                if( _str[wrapPoint] == ' ' ) {
                ...
                wrapInternal( _str.substr( wrapPoint ) ); return ;
            }
            if( _str[pos] == tabChar ) {
                nextTab = pos;
